...
exports.uploadFile = async (req, res) => {
  if (!req.file) {
    return res
      .status(400)
      .send({ message: "Por favor, faça upload de um arquivo." });
  }
  ...
  res
    .status(200)
    .send({ message: "Arquivo carregado com sucesso! Aguarde a validação." });
};
...
exports.validateFile = async (req, res) => {
  ...
  if (!fileContent) {
    return res
      .status(400)
      .send({ message: "Nenhum arquivo carregado para validação." });
  }
      ...
      for (let product of result.data) {
        if (product.cost_price && product.cost_price > product.sales_price) {
          validationState.errors.push(
            `Erro no produto de código ${product.code} - ${product.name}: O preço de custo não pode ser maior que o preço de venda.`
          );
        }

        if (!product.code || !product.new_price || !product.name) {
          validationState.errors.push(
            `Erro no produto de código${product.code} - ${product.name}: Campos necessários ausentes.`
          );
        }
        ...
        if (existingProduct) {
          ...
          if (
            product.new_price < 0.9 * currentPrice ||
            product.new_price > 1.1 * currentPrice
          ) {
            validationState.errors.push(
              `Erro no produto de código ${product.code} - ${product.name}: Reajuste de preço fora do limite permitido.`
            );
          }
          product.current_price = currentPrice;
        } else {
          validationState.errors.push(
            `Erro no produto de código ${product.code} - ${product.name}: Produto não existe no banco de dados.`
          );
        }
...
exports.updateProducts = async (req, res) => {
  try {
    ...
    for (let product of products) {
      console.log(
        `Iniciando a atualização/criação do produto com nome: ${product.name}`
      );
    ...
    res
      .status(500)
      .send({ message: "Erro ao atualizar produtos.", error: error.message });
  }
};
...
exports.listAllProducts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Erro ao listar produtos.", error: error.message });
  }
};
